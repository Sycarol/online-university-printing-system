<?php
  ...
  <body>
    ...
    <center>
      <form action="useraccountprinter.php" method="POST">
...
<div id="historyTableFilter">
  ...
  if(isset($_POST['submitHistoryFilter']))
  {
    ...
    if(isset($_POST['statusRequest']))
    {
        ...
        {
          $sql = "SELECT * FROM printing WHERE (printerID = '$printerid' AND status = '$statusRequest' AND dateRequest BETWEEN '$userhistoryfrom' AND '$userhistoryto' )";
        }
        else
        {
          $sql = "SELECT * FROM printing WHERE (printerID = '$printerid' AND status = '$statusRequest')";
        }
    }
    else
    {
      if(isset($userhistoryto) && isset($userhistoryfrom))
      {
        $sql = "SELECT * FROM printing WHERE (printerID = '$printerid' AND dateRequest BETWEEN '$userhistoryfrom' AND '$userhistoryto' )";
      }
      else
      {
        $sql = "SELECT * FROM printing WHERE (printerID = '$printerid')";
      }
      // code...
    }
